...
默认端口 5216（`PORT` 可以覆盖），数据存在 `data/db.json`，页面在 `/`。
...
4. **预警等级**：水位达到汛限/警戒要提级；**入库流量**达到 `inflowAttentionFlow`、`inflowSeriousFlow` 也要提级（两个输入都要看，不能只看水位）。水位与入库流量各判一级，**哪个先到门槛就按哪个定级**（同级则算两者共同），每一次都要写清是按哪个输入定的（水位 / 入库流量 / 水位+入库流量）；水位记录列表、水库详情、概览三处用同一套判断。
