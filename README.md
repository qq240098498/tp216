...
4. **预警等级**：水位达到汛限/警戒要提级；**入库流量**达到 `inflowAttentionFlow`、`inflowSeriousFlow` 也要提级（两个输入都要看，不能只看水位）。两个输入分别定级后**取高者**为最终等级，接口要给出本次是按哪个输入定的（水位 / 入库流量 / 两者同级），水位记录列表、水库详情、概览三处用同一套判断。
